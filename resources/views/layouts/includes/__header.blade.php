                ...
                <ul class="navbar-nav header-right">
                    <li class="nav-item dropdown notification_dropdown">
                        <a class="nav-link  ai-icon" href="javascript:;" role="button" data-toggle="dropdown">
                            ...
                            <span class="badge light text-white bg-primary">
                                ...
                                @elseif(Auth::user()->userDetail->role == 3 || Auth::user()->userDetail->role == 4)
                                    @php
                                        $pic = App\Models\Pic::with('users', 'projects')
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    @php
                                        if (Auth::user()->userDetail->role == 1 || Auth::user()->userDetail->role == 2) {
                                            ...
                                            $pic = App\Models\Pic::with('users', 'projects')
                                                ->where('user_id', Auth::id())
                                                ->pluck('project_id');
                                            $notes = App\Models\Note::with('projects', 'users')
                                                ->whereIn('project_id', $pic)
                                                ->paginate(5);
                                        }
                                    @endphp
                                        ...
                                        <li>
                                            <div class="timeline-panel">
                                                ...
                                                <div class="media-body">
                                                    @if (Auth::user()->userDetail->role == 1 || Auth::user()->userDetail->role == 2)
                                                        <a
                                                            href="{{ route('admin-task-detail', $item->projects->id) }}">
                                                            <h6 class="mb-1">
                                                                {{ ucwords(strtolower($item->projects->judul_project)) }}
                                                            </h6>
                                                        </a>
                                                    @elseif(Auth::user()->userDetail->role == 3)
                                                        <a
                                                            href="{{ route('lead-task-detail', $item->projects->id) }}">
                                                            <h6 class="mb-1">
                                                                {{ ucwords(strtolower($item->projects->judul_project)) }}
                                                            </h6>
                                                        </a>
                                                    @else
                                                        <a
                                                            href="{{ route('user-task-kumpul', $item->projects->id) }}">
                                                            <h6 class="mb-1">
                                                                {{ ucwords(strtolower($item->projects->judul_project)) }}
                                                            </h6>
                                                        </a>
                                                    ...
                                                    @endphp
                                                    <small
                                                        class="d-block">{{ \Carbon\Carbon::create($item->created_at)->diffForHumans() }}</small>
                                                </div>
                                            </div>
                                        </li>
                                    @empty
                            ...
                            <a class="all-notification" href="{{ route('notes-index') }}">See all notifications <i
